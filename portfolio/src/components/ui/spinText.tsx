...
const CircularText: React.FC<CircularTextProps> = ({
  ...
  return (
    <motion.div
      initial={{ rotate: 0 }}
      className={`mx-auto rounded-full w-[100px] h-[100px] text-primary text-center cursor-pointer origin-center ${className}`}
      animate={controls}
    ...
    >
      {letters.map((letter, i) => {
        ...
        return (
          <span
            key={i}
            className="absolute inline-block inset-0 text-base font-semibold transition-all duration-500 ease-[cubic-bezier(0,0,0,1)]"
